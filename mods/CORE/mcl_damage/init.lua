...
function mcl_damage.from_mt(mt_reason)
	...
	-- 🎯 【PvPパルス動的内部リレー大開通】：
	--     キックされたまさにその一瞬、C++側の高速文字列トリミング筋肉が実在すれば、
	--     すべての演算をC++レジスタ内へ放流し、Luaのヒープ（ガベージ）を一切汚さずに0ms完食！！！
	local api = rawget(_G, "mclcapi")
	if api and api.native_from_mt then
		mcl_reason = api.native_from_mt(mt_reason)
		
		-- ⚔️ 【安全フック】：もし原因が他者からの殴打（punch）だった場合は、
		--     型崩れを防ぐためにLua側の正規ルート（from_punch）へ安全にバケツリレー合流させる！
		if mcl_reason._mcl_trigger_punch then
			mcl_reason._mcl_trigger_punch = nil -- 記号フラグを引き算消去
			mcl_damage.from_punch(mcl_reason, mt_reason.object)
		end
	else
		-- 🛡️ 【鉄壁の保険】：C++未マウント時、および0手目のフライング時は、
		--     本家オリジナルの泥臭い前方一致検索（find）ループで即死を完全防犯ガード！
		if mt_reason._mcl_reason then
			mcl_reason = mt_reason._mcl_reason
		else
			mcl_reason = {type = "generic"}
