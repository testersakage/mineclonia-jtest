...
function image:constructor(pixels)
	...
	self.data = ""			-- c++ 新設
	self.pixel_depth = 32   -- c++ デフォルト32bit固定出荷
-- c++ check
--[[
	-- 全自動変数レントゲンスキャナー
	--     この個体が冒険のなかで読み書きされる「すべての内部変数（プロパティ）」を、
	--     debug.txt へ自動インベントリ（ダンプ）させる！
	local real_data = {
		pixels = self.pixels,
		width = self.width,
		height = self.height,
	...
	-- selfの中身を一度空っぽにし、盗聴Proxyに変装させる
	for k, v in pairs(self) do if k ~= "constructor" then self[k] = nil end end

	setmetatable(self, {
		__index = function(t, key)
			-- どの変数（プロパティ）がシステムから「参照（既知）」されたかをログ取り！
			minetest.log("action", "[TGA SCANNER] READ -> " .. tostring(key))
			return real_data[key] or image[key] -- 本物のデータかメソッドをリレー
		end,
		__newindex = function(t, key, value)
			-- 誰かが新しい設定値や「未知の変数」を書き込んできた瞬間を捕獲
			minetest.log("action", "[TGA SCANNER] WRITE !! -> " .. tostring(key) .. " = " .. tostring(value))
			real_data[key] = value
		end
	})
	-- ─── 【スキャナー仕込みここまで】 ───
...
function image:save(filename, properties)
	...
	if is_known_format and is_known_compression and is_valid_pixels then
		...
		-- C++側のスタックを絶対に狂わせないよう、
		-- 重い2次元 [z][x] マトリクスを、Lua側のテーブルで「1重のフラットARGB数値配列」へ
		local flat_pixels = {}
		local count = 1
		for z = 1, self.height do
			local row = self.pixels[z]
			if row then
				for x = 1, self.width do
					local pixel = row[x]
					if pixel then
						local r = pixel[1] or 255
						local g = pixel[2] or 255
		...
		-- 【安全出荷】：1重に整えられたピクセル配列と、ハだかの設定値だけを C++ 側へ
		if mclcapi and mclcapi.native_tga_encode then
			local success = mclcapi.native_tga_encode(filename, self.width, self.height, flat_pixels, color_format, compression)
			if success then return true end
		end
	else
		-- 【未知の異物を検知】：C++側を汚さず、Lua側の安全なエラーログへフォールバック
		minetest.log("warning", "[TGA EXCEPTION] UNKNOWN DATA DETECTED! Format: " .. tostring(fmt) .. " | Comp: " .. tostring(comp))
		minetest.log("warning", " -> Format: " .. tostring(color_format) .. " | Compression: " .. tostring(compression))
		
		-- 保険（フォールバック）として、古い生Lua版の重い直列ループを走らせてゲームの即死だけは絶対防犯ガード！
		self:encode_old_lua_backup(properties)
	end
end
