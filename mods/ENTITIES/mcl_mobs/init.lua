...
-- C++側（mclcapi）に焼き固めた職歴・判定・取引APIが実在するなら、
-- Lua側の既存の register_villager 名義の入り口を丸ごとC++側へ直撃リダイレクト出荷！！！
local l_capi = rawget(_G, "mclcapi") or {}
local l_mobs = rawget(_G, "mobs_mc") or {}

if l_capi.native_register_villager then
	if l_mobs.register_villager then
		local original_register_villager = l_mobs.register_villager
		l_mobs.register_villager = function(profession, poi, trades, gifts)
			local res = l_capi.native_register_villager(profession, poi, trades, gifts)
			return original_register_villager(res, poi, trades, gifts)
		end
	end

	if l_mobs.native_check_poi_valid then
		l_mobs.check_poi_valid = l_capi.native_check_poi_valid
	end

	if l_mobs.native_filter_trades then
		l_mobs.filter_trades = l_capi.native_filter_trades
	end
	
	minetest.log("action", "[mcl_mobs/init.lua] 3 Mob AI C++ APIs gloriously hijacked and synchronized into mclcapi.")
end

-- AI思考スピード動的ハッキングコマンド /pathfinding_speed
core.register_chatcommand("pathfinding_speed", {
	params = "<seconds>",
	description = "Globally change the native C++ mob pathfinding execution tick interval.",
	privs = { server = true },
	func = function(name, param)
		local num = tonumber(param)
		if num and mclcapi and mclcapi.native_set_pathfinding_speed then
			local current = mclcapi.native_set_pathfinding_speed(num)
			return true, "[MCL DEBUG] Global mob pathfinding execution interval adjustments verified: " .. current .. " seconds."
		end
		return false, "Invalid execution seconds."
	end,
})
