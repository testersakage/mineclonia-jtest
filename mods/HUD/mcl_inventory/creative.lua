...
function mcl_inventory.set_creative_formspec(player)
	...
	if name == "inv" then
		...
		-- TODO: Remove as soon as support for versions prior to 5.11 is dropped
		local version = core.get_version().string
		local s_major, s_minor = version:match("(%d+)%.(%d+)")
		local n_major, n_minor = tonumber(s_major), tonumber(s_minor)

		if n_major < 5 or (n_major == 5 and n_minor < 11) then
			core.settings:set_bool("enable_inventory_scrollbar", false)
		end

		if core.settings:get_bool("enable_inventory_scrollbar") then
