	...
	if definitions.tiles and definitions.tiles[1] then
		...
		for wood_key, plank in pairs(PLANK_TEXTURES) do
			if t:find(wood_key) then
				...
				core.log("action", "[mcl_fences] mapped tile '"..t.."' -> plank '"..plank.."' for '"..tostring(definitions.description or "").."'")
				return
			end
		end
	end
...
local tpl_fences = {
	_doc_items_longdesc = S("Fences are structures which block the way. Fences will connect to each other and solid blocks."),
	paramtype = "light",
...
local tpl_fence_gates = {
	...
	_doc_items_longdesc = S("Fence gates can be opened or closed. Fences will connect nicely to fence gates."),
	_doc_items_usagehelp = S("Right-click to open/close the gate."),
	paramtype = "light",
	is_ground_content = false,
	paramtype2 = "facedir",
	sunlight_propagates = true,
	...
	on_construct = function(pos) core.get_meta(pos):set_int("state", 0) end,
	_mcl_redstone = {
		...
		update = function(pos)
			if mcl_redstone and mcl_redstone.get_power and mcl_redstone.get_power(pos) ~= 0 then
				punch_gate(pos, core.get_node(pos))
			end
		end,
		init = function() end,
	},
	...
	_on_wind_charge_hit = function(pos) punch_gate(pos, core.get_node(pos)); return true end
}
...
local tpl_fence_gates_open = {
	...
	selection_box = { type = "fixed", fixed = {{ -0.5, -0.1875, -0.0625, 0.5, 0.5, 0.0625 }} },
	on_rightclick = function(pos, node, _) punch_gate(pos, node) end,
...
function mcl_fences.register_fence_def(name, definitions)
	...
	if definitions.tiles and definitions.tiles[1] then
		...
		if definitions.tiles and definitions.tiles[1] then
			...
			core.log("warning", "[mcl_fences] register_fence_def: '"..tostring(name).."' has no tiles.")
		end
	end
