...
minetest.register_node("mcl_lush_caves:rooted_dirt", {
	...
	on_rightclick = function(pos, node, clicker, itemstack, pointed_thing)
		...
		if minetest.get_item_group(itemstack:get_name(), "hoe") then
			local node_above = minetest.get_node(pointed_thing.above)
			local pos_to_add = pointed_thing.above
			if node_above.name ~= "air" then
				pos_to_add = clicker:get_pos()
			end
			minetest.add_item(pos_to_add, ItemStack("mcl_lush_caves:hanging_roots"))
		end
