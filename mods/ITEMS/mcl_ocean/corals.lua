...
local function coral_on_place(itemstack, placer, pointed_thing)
	...
	local g_block = core.get_item_group(node_under.name, "coral_block")
	local g_coral = core.get_item_group(itemstack:get_name(), "coral")
	local g_species_block = core.get_item_group(node_under.name, "coral_species")
	local g_species_plant = core.get_item_group(itemstack:get_name(), "coral_species")
	...
	node_under.param2 = core.registered_items[node_under.name].place_param2 or 1
	...
	local def_node = core.registered_nodes[node_under.name]
...
for c=1, #corals do
	...
	-- Coral Block
	core.register_node("mcl_ocean:"..id.."_coral_block", {
	...
	core.register_node("mcl_ocean:dead_"..id.."_coral_block", {
	...
	-- Coral
	core.register_node("mcl_ocean:"..id.."_coral", {
		...
		after_dig_node = function(pos)
			...
			if core.get_item_group(node.name, "coral") == 0 then
				core.set_node(pos, {name="mcl_ocean:"..id.."_coral_block"})
			end
		end,
		_mcl_hardness = 0,
	...
	core.register_node("mcl_ocean:dead_"..id.."_coral", {
		...
		after_dig_node = function(pos)
			local node = core.get_node(pos)
			if core.get_item_group(node.name, "coral") == 0 then
				core.set_node(pos, {name="mcl_ocean:dead_"..id.."_coral_block"})
			end
		end,
		_mcl_hardness = 0,
		_mcl_blast_resistance = 0,
	...
	-- Coral Fan
	core.register_node("mcl_ocean:"..id.."_coral_fan", {
...
core.register_abm({
	...
	action = function(pos, node)
		...
		if coral_state == 1 then
			...
			if core.get_node({x=pos.x, y=pos.y+1, z=pos.z}).name ~= "mcl_core:water_source" then
...
core.register_abm({
	...
	action = function(pos, node)
		...
		if coral_state == 1 then
			...
			for p=1, #posses do
				local checknode = core.get_node(vector.add(pos, posses[p]))
