...
function mcl_potions._load_player_effects(player, ef_cache) --passing the local EF so it can be used in mcl_potions_compat
	ef_cache = ef_cache or EF
	if not player:is_player() then
		return
	end
	local meta = player:get_meta()
	...
	for name, effect in pairs(registered_effects) do
		...
		if loaded then
			ef_cache[name][player] = loaded
		end
		if ef_cache[name][player] then -- this is needed because of legacy effects loaded separately
			if effect.uses_factor and type(ef_cache[name][player].factor) ~= "number" then
				EF[name][player].factor = effect.level_to_factor(1)
			end
			if effect.on_load then
				effect.on_load(player, ef_cache[name][player].factor)
			end
		end
	end
end
