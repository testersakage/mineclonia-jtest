...
core.register_on_mods_loaded(function()
	for n, _ in pairs(core.registered_nodes) do
		...
		elseif minetest.get_item_group(n, "random4dir") > 0 then
			table.insert(random4dir_nodes, n)
		end
	end
...
local function set_param2_nodes(vm, data, data2, emin, emax, area, minp, maxp, blockseed) ---@diagnostic disable-line: unused-local
	...
	local nodes = minetest.find_nodes_in_area(minp, maxp, random4dir_nodes)
	for _, n in pairs(nodes) do
		local p_pos = area:index(n.x, n.y, n.z)
		data2[p_pos] = pcgrandom:next(0, 3)
		lvm_used = true
	end
	return lvm_used
end
