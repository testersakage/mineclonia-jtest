...
local disabled_structures = {}
-- FIXME: switch to vl_structures_disabled?
if minetest.settings:get("mcl_disabled_structures") then
	for _, v in pairs(minetest.settings:get("mcl_disabled_structures"):split(",")) do
		disabled_structures[v] = v
	end
end
function vl_structures.is_disabled(structname)
	return disabled_structures[structname] ~= nil
end
...
--[[minetest.register_lbm({
	name = "vl_structures:struct_lbm",
	run_at_every_load = true,
	nodenames = {"group:structblock_lbm"},
	action = function(pos, node)
		minetest.remove_node(pos)
		local name = node.name:gsub("vl_structures:structblock_","")
		local def = vl_structures.registered_structures[name]
		if not def then return end
		local pr = PcgRandom(minetest.hash_node_position(pos) + worldseed + RANDOM_SEED_OFFSET)
		vl_structures.place_structure(pos, def, pr, pr.next()) -- TODO: remove blockseeds?
	end
})]]
