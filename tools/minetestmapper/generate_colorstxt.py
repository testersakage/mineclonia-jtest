...
from PIL import Image, ImageChops, ImageColor, ImageEnhance, ImageMath  # aka "pillow"
from pyparsing import CharsNotIn, Suppress, ZeroOrMore, infix_notation, opAssoc
...
REPLACEMENTS = [
    ...
    for pat, rule in [
        ...
        (
            r"^(default|mclx?_core):river_water_(flowing|source)$",
            (36, 67, 130, 224, 128),
        ),
        (
            r"^(default|mclx?_core):water_(flowing|source)$",
            adjust_water_transparency,
        ),  # was (36, 67, 130, 224, 128)),
        ...
        (
            r"^mcl_bamboo:bamboo(_endcap)?$",
            lambda c: (c[0], c[1], c[2], 32),
        ),  # much more alpha
    ]
]
...
class Filter:
    ...
    def gen(self, prev=None):
        ...
        if self.fname in ["combine"]:
            ...
            # print(self.fname, self.opts)
            assert self.opts[0] == "0", (
                "Color shifts are currently not implemented."
            )  ## TODO
            assert len(self.opts) == 2, (
                "Only saturation is currently supported."
            )  ## TODO
            f = int(self.opts[1])
            return ImageEnhance.Color(prev).enhance(f / 100.0 + 1)
            ...
            return Image.merge(
                "RGBA",
                [
                    ImageMath.unsafe_eval("a&b", a=a, b=b).convert("L")
                    for a, b in zip(prev.split(), m.split())
                ],
            )
...
try:
    ...
    if not os.path.isdir(os.path.join(gamepath, "mods")):
        print(
            f"'{gamepath}' doesn't exist or does not contain a game.", file=sys.stderr
        )
        exit(1)
...
for line in fin:
    ...
    if len(line) == 3 and line[2].startswith("#"):
        ...
        if line[3].startswith("[combine:16x2:0,0="):
            line[3] = line[3][
                len("[combine:16x2:0,0=") :
            ]  # simple resize for colorwallmounted
...
cmap = dict(
    (x, y) for x, y in sorted(cmap.items()) if y
)  # remove remaining null entries
...
for node, color in cmap.items():
    ...
    if not isinstance(color[0], int):
        color = color[0]  # param2 needs minetestmapper support first
...
js = re.sub(
    r'\n\t*(?!["\t}])', " ", js
)  # partially undo indenting to make more compact
